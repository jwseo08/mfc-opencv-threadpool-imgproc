...
스레드풀과 별도로 하나의 독립적인 작업을 실행할 수 있는 싱글 스레드도 지원하며, 스레드풀 작업과 싱글 스레드 작업을 독립적으로 관리합니다.
...
4. 스레드풀 사용 시 워커 스레드 수를 설정합니다.
5. OpenCV 내부 스레드 사용 여부를 설정합니다.
6. 이미지 처리 작업을 실행합니다.
7. 처리 결과와 작업 시간을 프로그램 화면에서 확인합니다.
8. 이미지 리스트를 더블 클릭하면 확대된 이미지를 확인할 수 있습니다.
