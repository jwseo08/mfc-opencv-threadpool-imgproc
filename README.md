...
<p align="center">
  MFC 기반 영상 전처리 및 Thread Pool 병렬 처리 성능 비교 애플리케이션
</p>

## 프로젝트 소개

Windows MFC와 OpenCV를 기반으로 구현한 영상 처리 데모 애플리케이션입니다.

입력 이미지에 대해 명암 보정, 경계선 및 모서리 검출, 기울기 보정 등의 전처리를 수행하며, 이미지의 가독성을 높이고 텍스트 추출에 적합한 형태로 보정합니다.

영상 처리 성능을 개선하기 위해 고정 크기의 Thread Pool을 구현하고, 다수의 이미지 처리 작업을 Task Queue에 등록하여 Worker Thread에서 병렬로 처리하도록 구성했습니다.
...
UI는 MFC를 기반으로 구성했으며, 기본 컨트롤을 상속한 커스텀 컨트롤을 적용했습니다.

영상 처리 작업은 UI Thread와 분리된 Worker Thread에서 수행하며, Worker Thread가 MFC UI 컨트롤에 직접 접근하지 않도록 Windows Message 기반의 구조를 사용했습니다.
