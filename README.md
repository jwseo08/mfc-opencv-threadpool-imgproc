...
또한 Worker Thread 수를 변경하면서 처리 시간의 변화를 확인하고, OpenCV 내부 스레드 사용 여부가 전체 처리 시간에 미치는 영향도 함께 비교했습니다.
...
이는 일정 수준 이상에서는 병렬 처리 이점이 줄어들고, 스레드 관리 비용과 CPU 자원 경합의 영향이 커질 수 있음을 보여줍니다.
...
|---|---:|---:|
| 순차 처리 | 29.16초 | 29.86초 |
| Thread Pool 6 | 6.14초 | 6.11초 |
| Thread Pool 9 | 5.25초 | 5.24초 |
| Thread Pool 12 | 4.84초 | 4.85초 |
| Thread Pool 18 | 4.81초 | 4.81초 |
| Thread Pool 24 | 5.00초 | 4.88초 |
| Thread Pool 36 | 4.90초 | 4.89초 |

### 결과 해석

- 순차 처리 대비 Thread Pool 병렬 처리에서 전체 처리 시간이 크게 감소했습니다.
- Worker Thread 수는 **6개에서 12개까지** 증가할 때 성능 향상이 비교적 뚜렷했습니다.
...
- 이는 Thread Pool의 Worker Thread와 OpenCV 내부 병렬 처리가 동시에 동작하더라도, 현재 환경에서는 성능 차이가 제한적이었음을 의미합니다.

### 정리

본 테스트에서는 **Thread Pool 기반 병렬 처리만으로도 순차 처리 대비 큰 성능 향상**을 확인할 수 있었습니다.  
반면 Worker Thread 수를 과도하게 늘린다고 해서 성능이 계속 비례하여 증가하지는 않았으며, 일정 수준 이후에는 **병렬화 이점보다 자원 경합과 관리 비용의 영향이 커질 수 있음**을 확인했습니다.

따라서 이미지 처리와 같은 CPU 중심 작업에서는, 실행 환경의 논리 프로세서 수와 작업 특성을 고려하여 **적절한 Worker Thread 수를 선택하는 것이 중요합니다.**
